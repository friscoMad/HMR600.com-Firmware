	...
	if ($mydir != "/tmp/myshare"){
		...
		if((substr($mydir, -4) == ".smb") || (substr($mydir, -4) == ".nfs") and ($output1[0] == "")){
			...
			if ($result != 0){
				...
				echo "<script>location.href='zip.php?dir=$uplink';</script>";
			}else if($id != ''){
				echo "<script>location.href='zip.php?dir=$mydir';</script>";
			}
		}
...
function change(){
	...
	if (aaa){
		...
		document.filelist.action = 'zip_file.php?dir=<? echo $mediapath;?>';
...
<body marginwidth=0 marginheight=0 leftmargin=0 topmargin=0 oncontextmenu="return false;" onunLoad="window.opener.location.reload();">
	<table width="100%" height="50" background="dlf/top_menu.jpg" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td width=850 height="50" align=middle><font face="arial" color="#ff0000"><h2>Archive</h2></font></td>
	    </tr>
	</table>
...
?>
	...
	<tr><td width=340>&nbsp <font color=white face='Arial' size='2'><?echo $currentpath; ?>/</td>
		<td><input type=button class='btn_2' onclick="javascript:change()" class="web-button" value="Archive"> </td>
