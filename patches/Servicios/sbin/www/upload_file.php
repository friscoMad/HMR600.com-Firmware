...
// Settings
		...
		if ((substr($_GET['dir'],0,2) != '/.') and (substr($_GET['dir'],0,1) != '.') and ($_GET["dir"]=='')){
			$save_path = '/tmp/ramfs/volumes/C:/';
		}else if ((substr($_GET['dir'],0,2) != '/.') and (substr($_GET['dir'],0,1) != '.') and (substr($_GET['dir'],0,12) =='/tmp/myshare')){
			$save_path = $_GET['dir']."/";
...
//echo $_GET["dir"]."------".$save_path."Hello";
// Process the file
        //sleep(50);
		//$save_path = utf8_encode($save_path);
		//$save_path = stripslashes($save_path);
		//$file_name = stripslashes($file_name);
        if (!@move_uploaded_file($_FILES[$upload_name]["tmp_name"], $save_path.$file_name)) {
                HandleError("File could not be saved.");
				//HandleError("$save_path");
                exit(0);
        }
