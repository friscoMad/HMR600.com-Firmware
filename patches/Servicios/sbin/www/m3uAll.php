...
function addlist(){
	...
	document.playlist.action="./m3uAllList.php?dir=<? echo $mediapath;?>";
...
function removelist(){
	...
	document.mylist.action="./m3uAllListremove.php?dir=<? echo $mediapath;?>";
...
/*function Restart(){
	document.mylist.target = 'gframe';
	document.mylist.action = 'restart.php';
	document.mylist.submit();
}

function Playit(){
	document.mylist.target = 'gframe';
	document.mylist.action = 'play.php?dir=playAll';
	document.mylist.submit();
}*/
</script>
...
<body marginwidth=0 marginheight=0 leftmargin=0 topmargin=0 oncontextmenu="return false;">
	<table width="987" height="94" background="dlf/top_menu.jpg" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td width=30%><img src="dlf/icon_other.jpg" height="94" width="189"/></td>
          	<td width=40% align="center"><font face="arial" color="#ff0000"><h2><?echo $STR_MyListAllTitle;?></h2></font></td>
	...
	if (($files[$x] != '.') and ($files[$x] != "..")) {
		...
		if(!is_dir($mydir . "/" . $files[$x])) {
			 if ((strtolower(strrchr($files[$x],'.')) == ".wma")||(strtolower(strrchr($files[$x],'.')) == ".mp3")||(strtolower(strrchr($files[$x],'.')) == ".wav")||(strtolower(strrchr($files[$x],'.')) == ".mp2")||(strtolower(strrchr($files[$x],'.')) == ".aac")||(strtolower(strrchr($files[$x],'.')) == ".ac3")||(strtolower(strrchr($files[$x],'.')) == ".dts")||(strtolower(strrchr($files[$x],'.')) == ".png")||(strtolower(strrchr($files[$x],'.')) == ".jpeg")|| (strtolower(strrchr($files[$x],'.')) == ".jpg")||(strtolower(strrchr($files[$x],'.')) == ".bmp")||(strtolower(strrchr($files[$x],'.')) == ".gif")||(strtolower(strrchr($files[$x],'.')) == ".tif")||(strtolower(strrchr($files[$x],'.')) == ".wmv")||(strtolower(strrchr($files[$x],'.')) == ".mpg")|| (strtolower(strrchr($files[$x],'.')) == ".avi") ||(strtolower(strrchr($files[$x],'.')) == ".dat")||(strtolower(strrchr($files[$x],'.')) == ".mpeg")||(strtolower(strrchr($files[$x],'.')) == ".divx")||(strtolower(strrchr($files[$x],'.')) == ".xvid")||(strtolower(strrchr($files[$x],'.')) == ".mkv")||(strtolower(strrchr($files[$x],'.')) == ".mov")||(strtolower(strrchr($files[$x],'.')) == ".asf")||(strtolower(strrchr($files[$x],'.')) == ".ts")||(strtolower(strrchr($files[$x],'.')) == ".ogm")||(strtolower(strrchr($files[$x],'.')) == ".mp4")
				 ||(strtolower(strrchr($files[$x],'.')) == ".flv")||(strtolower(strrchr($files[$x],'.')) == ".m4v") ||(strtolower(strrchr($files[$x],'.')) == ".flac")){
				...
				echo "<tr> <td>";
				echo "<table cellspacing='1' cellpadding='0' border='0'><tr><td><input type='checkbox' name='playlist[]' value='$files[$x]'>";
...
?>
		...
		<td align=middle>
			  ...
			  <form id='mylist' name='mylist' method='post' enctype='multipart/form-data'>
...
<table border=0 width=955><tr><td width=850>&nbsp
		...
		<!--td align=right><input type=button class='btn_2' onclick="javascript:Playit();" value="Play"></td>
		<td align=right><input type=button class='btn_2' onclick="javascript:Restart();" value="Reboot"></td-->
		<td align=right><input type=button class='btn_2' onclick="window.close()" value="<?echo $STR_Close;?>"></td>
		</tr>
</table>
</center>
